...
x_sample = sample(50, 0)
#code: "x_sample = map2(50, 0, \(x, y) sample(ntest = x, mu = y))" cannot work. Because it creates "list of 1" rather than "list of 50".
...
```
    ...
    ##  8     0     8  -0.179   0.385 
    ##  9     0     9   0.242   0.204 
    ## 10     0    10  -0.381   0.0379
    ## # ℹ 40 more rows

``` r
mu_1to6 = tibble()
for (i in 1:6) {
  res = map2(x_sample, i, \(x, y) t_test(sample = x, mu = y))
  result =
    tibble(res) |>
    unnest(res) |>
    rename(p_value = p.value) |>
